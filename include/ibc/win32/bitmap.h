...
// =============================================================================
/*!
  \file     ibc/image/bitmap.h
  \author   Dairoku Sekiguchi
  \version  1.0.0
  \date     2018/03/10
  \brief    Header file for handling the bitmap

  This file defines the bitmap  class for the IBC Library
*/


#ifndef IBC_IMAGE_BITMAP_H_
#define IBC_IMAGE_BITMAP_H_
...
namespace ibc
{
 namespace image
 {
  ...
  public:
    ...
    // setBitmapInfo
    // ---------------------------------------------------------------------
    void  setBitmapInfo(int inWidth, int inHeight, int inBitCount = 24)
    {
      ...
      if (allocationIsNeeded == true)
      {
        ...
        if (mBitmapInfoPtr != NULL && mAllocatedBitmapInfoPtr == NULL)
          throw ImageException(Exception::PARAM_ERROR,
                "invalid parameter (can't change inBitCount)", IBC_EXCEPTION_LOCATION_MACRO, 0);
    ...
    // setBitmapBits
    // ---------------------------------------------------------------------
    void  setBitmapBits(unsigned char *inImageBuffer, size_t inBufferSize,
                int inWidth, int inHeight, int inBitCount = 24)
    {
      setBitmapInfo(inWidth, inHeight, inBitCount);
      setBitmapBits(inImageBuffer, inBufferSize);
    }
    // ---------------------------------------------------------------------
    // setBitmapBits
    // ---------------------------------------------------------------------
    void  setBitmapBits(unsigned char *inImageBuffer, size_t inBufferSize)
    {
      ...
      if (inBufferSize != mBitmapBitsSize)
        throw ImageException(Exception::PARAM_ERROR,
              "invalid inBufferSize", IBC_EXCEPTION_LOCATION_MACRO, 0);
    ...
    bool  saveToFile(const LPCTSTR inFileName)
    {
      ...
      if (fileHandle == INVALID_HANDLE_VALUE)
      {
        if (mThrowsEx == false)
          ...
          throw ImageException(Exception::OS_ERROR,
                "::CreateFileW() returned NULL", IBC_EXCEPTION_LOCATION_MACRO, ::GetLastError());
      }
      ...
      if (result == 0 || sizeInBytes != sizeof(BITMAPFILEHEADER))
      {
        ...
        if (mThrowsEx == false)
          ...
          throw ImageException(Exception::OS_ERROR,
                "::WriteFile() returned an error", IBC_EXCEPTION_LOCATION_MACRO, ::GetLastError());
      }
    ...
    static Bitmap  *loadFromFile(const LPCTSTR inFileName, bool inThrowsEx = false)
    {
      ...
      if (fileHandle == INVALID_HANDLE_VALUE)
      {
        if (inThrowsEx == false)
          ...
          throw ImageException(Exception::OS_ERROR,
                "::CreateFile() returned NULL", IBC_EXCEPTION_LOCATION_MACRO, ::GetLastError());
      }
      ...
      if (result == 0 || sizeInBytes != sizeof(BITMAPFILEHEADER))
      {
        ...
        if (inThrowsEx == false)
          ...
          throw ImageException(Exception::OS_ERROR,
                "::ReadFile() returned an error", IBC_EXCEPTION_LOCATION_MACRO, ::GetLastError());
      }
      ...
      if (bmpFHeader.bfType != 0x4d42 || bmpFHeader.bfReserved1 != 0 || bmpFHeader.bfReserved2 != 0)
      {
        ...
        if (inThrowsEx == false)
          ...
          throw ImageException(Exception::FILE_FORMAT_ERROR,
                "bmpFHeader.bfType != 0x4d42 || bmpFHeader.bfReserved1 != 0 || bmpFHeader.bfReserved2 != 0",
                IBC_EXCEPTION_LOCATION_MACRO, ::GetLastError());
      }
      ...
      if (bitmap == NULL)
      {
        ...
        if (inThrowsEx == false)
          ...
          throw ImageException(Exception::MEMORY_ERROR,
                "new Bitmap(inThrowsEx) returned NULL", IBC_EXCEPTION_LOCATION_MACRO, ::GetLastError());
      }
      ...
      *bitmap->mBitmapInfoPtr = bmpInfo;
        ...
        if (bmpFHeader.bfOffBits != offBits)
        {
          ...
          if (filePointer == INVALID_SET_FILE_POINTER)
          {
            ...
            if (inThrowsEx == false)
              ...
              throw ImageException(Exception::OS_ERROR,
                    "::SetFilePointer() returned an error", IBC_EXCEPTION_LOCATION_MACRO, ::GetLastError());
          }
        }
      }
      ...
      if (bitmap->mBitmapBitsPtr == NULL)
      {
        ...
        if (inThrowsEx == false)
          ...
          throw ImageException(Exception::MEMORY_ERROR,
                "new unsigned char[bitmap->mBitmapBitsSize] returned NULL", IBC_EXCEPTION_LOCATION_MACRO, ::GetLastError());
      }
    ...
    bool  allocateBitmapInfo(int inColorPalletNum = 0)
    {
      ...
      if (mAllocatedBitmapInfoPtr == NULL)
      {
        if (mThrowsEx == false)
          ...
          throw ImageException(BleuException::MEMORY_ERROR,
                "new unsigned char[mBitmapInfoSize] returned NULL", IBC_EXCEPTION_LOCATION_MACRO, ::GetLastError());
      }
    ...
    bool  allocateImageBuffer()
    {
      ...
      if (mAllocatedBitmapBitsPtr == NULL)
      {
        if (mThrowsEx == false)
          ...
          throw ImageException(BleuException::MEMORY_ERROR,
                "new unsigned char[mBitmapBitsSize] returned NULL", IBC_EXCEPTION_LOCATION_MACRO, ::GetLastError());
      }
...
#endif  // #ifdef IBC_IMAGE_BITMAP_H_
